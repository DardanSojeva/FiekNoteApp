...
import fiek.ds.android.fieknote.async.notes.NoteLoaderTask;
import fiek.ds.android.fieknote.async.notes.NoteProcessorArchive;
import fiek.ds.android.fieknote.async.notes.NoteProcessorCategorize;
import fiek.ds.android.fieknote.async.notes.NoteProcessorTrash;
import fiek.ds.android.fieknote.utils.KeyboardUtils;
...
import fiek.ds.android.fieknote.models.UndoBarController;
import fiek.ds.android.fieknote.utils.AnimationsHelper;
...
import fiek.ds.android.fieknote.async.bus.NotesMergeEvent;
import fiek.ds.android.fieknote.utils.TagsHelper;
import fiek.ds.android.fieknote.utils.TextHelper;
import fiek.ds.android.fieknote.db.DbHelper;
import fiek.ds.android.fieknote.helpers.NotesHelper;
...
import fiek.ds.android.fieknote.models.adapters.NoteAdapter;
import fiek.ds.android.fieknote.models.holders.NoteViewHolder;
import fiek.ds.android.fieknote.models.listeners.OnViewTouchedListener;
import fiek.ds.android.fieknote.models.views.Fab;
import fiek.ds.android.fieknote.models.views.InterceptorLinearLayout;
...
public class ListFragment extends BaseFragment implements OnViewTouchedListener, UndoBarController.UndoListener {
    ...
    @Bind(fiek.ds.android.fieknote.R.id.list_root) InterceptorLinearLayout listRoot;
    @Bind(fiek.ds.android.fieknote.R.id.list) DynamicListView list;
    @Bind(fiek.ds.android.fieknote.R.id.search_layout) View searchLayout;
    @Bind(fiek.ds.android.fieknote.R.id.search_query) android.widget.TextView searchQueryView;
    @Bind(fiek.ds.android.fieknote.R.id.search_cancel) ImageView searchCancel;
    @Bind(fiek.ds.android.fieknote.R.id.empty_list) TextView empyListItem;
    @Bind(fiek.ds.android.fieknote.R.id.expanded_image) ImageView expandedImageView;
    @Bind(fiek.ds.android.fieknote.R.id.fab)  View fabView;
    @Bind(fiek.ds.android.fieknote.R.id.undobar) View undoBarView;
    @Bind(fiek.ds.android.fieknote.R.id.progress_wheel) ProgressWheel progress_wheel;
	@Bind(fiek.ds.android.fieknote.R.id.snackbar_placeholder) View snackBarPlaceholder;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        View view = inflater.inflate(fiek.ds.android.fieknote.R.layout.fragment_list, container, false);
	...
	private void requestReadStoragePermission() {
		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN) {
			PermissionsHelper.requestPermission(mainActivity, Manifest.permission.READ_EXTERNAL_STORAGE, fiek.ds.android.fieknote.R
    ...
    private void initFab() {
        ...
        fab.setOnFabItemClickedListener(id -> {
            ...
            switch (id) {
                case fiek.ds.android.fieknote.R.id.fab_expand_menu_button:
                    editNote(new Note(), v);
                    break;
                case fiek.ds.android.fieknote.R.id.fab_note:
                    editNote(new Note(), v);
                    break;
                case fiek.ds.android.fieknote.R.id.fab_camera:
                    Intent i = mainActivity.getIntent();
                    i.setAction(Constants.ACTION_TAKE_PHOTO);
                    mainActivity.setIntent(i);
                    editNote(new Note(), v);
                    break;
                case fiek.ds.android.fieknote.R.id.fab_checklist:
                    Note note = new Note();
        ...
        fab.setOverlay(fiek.ds.android.fieknote.R.color.white_overlay);
    }
    ...
    private void initTitle() {
        String[] navigationList = getResources().getStringArray(fiek.ds.android.fieknote.R.array.navigation_list);
        String[] navigationListCodes = getResources().getStringArray(fiek.ds.android.fieknote.R.array.navigation_list_codes);
        ...
        title = title == null ? getString(fiek.ds.android.fieknote.R.string.title_activity_list) : title;
    ...
    private void stopJingles() {
        if (jinglesAnimation != null) {
            ...
            empyListItem.setCompoundDrawablesWithIntrinsicBounds(0, fiek.ds.android.fieknote.R.drawable.jingles_animation, 0, 0);
    ...
    private void refreshListScrollPosition() {
        if (list != null) {
            ...
            listViewPositionOffset = (v == null) ? (int) getResources().getDimension(fiek.ds.android.fieknote.R.dimen.vertical_margin) : v.getTop();
        }
    }
    ...
    private final class ModeCallback implements android.support.v7.view.ActionMode.Callback {
        ...
        public boolean onCreateActionMode(ActionMode mode, Menu menu) {
            ...
            inflater.inflate(fiek.ds.android.fieknote.R.menu.menu_list, menu);
            actionMode = mode;
        ...
        public void onDestroyActionMode(ActionMode mode) {
            ...
            for (int i = 0; i < listAdapter.getSelectedItems().size(); i++) {
                ...
                if (listAdapter.getCount() > key && listAdapter.getItem(key) != null && v != null) {
                    listAdapter.restoreDrawable(listAdapter.getItem(key), v.findViewById(fiek.ds.android.fieknote.R.id.card_layout));
                }
            }
        ...
        public boolean onActionItemClicked(final ActionMode mode, final MenuItem item) {
            Integer[] protectedActions = {fiek.ds.android.fieknote.R.id.menu_select_all, fiek.ds.android.fieknote.R.id.menu_merge};
    ...
    private void toggleListViewItem(View view, int position) {
        ...
        LinearLayout cardLayout = (LinearLayout) view.findViewById(fiek.ds.android.fieknote.R.id.card_layout);
        if (!getSelectedNotes().contains(note)) {
            getSelectedNotes().add(note);
            listAdapter.addSelectedItem(position);
            cardLayout.setBackgroundColor(getResources().getColor(fiek.ds.android.fieknote.R.color.list_bg_selected));
        } else {
    ...
    private ImageView getZoomListItemView(View view, Note note) {
        if (expandedImageView != null) {
            ...
            if (note.getAttachmentsList().size() > 0) {
                targetView = view.findViewById(fiek.ds.android.fieknote.R.id.attachmentThumbnail);
            }
            if (targetView == null && note.getCategory() != null) {
                targetView = view.findViewById(fiek.ds.android.fieknote.R.id.category_marker);
            }
            if (targetView == null) {
    ...
    private void initSortingSubmenu() {
        final String[] arrayDb = getResources().getStringArray(fiek.ds.android.fieknote.R.array.sortable_columns);
        final String[] arrayDialog = getResources().getStringArray(fiek.ds.android.fieknote.R.array.sortable_columns_human_readable);
        int selected = Arrays.asList(arrayDb).indexOf(prefs.getString(Constants.PREF_SORTING_COLUMN, arrayDb[0]));

        SubMenu sortMenu = this.menu.findItem(fiek.ds.android.fieknote.R.id.menu_sort).getSubMenu();
    ...
    private void prepareActionModeMenu() {
        ...
        if (navigation == Navigation.TRASH) {
            ...
            if (getSelectedCount() == 1) {
                menu.findItem(fiek.ds.android.fieknote.R.id.menu_share).setVisible(true);
                menu.findItem(fiek.ds.android.fieknote.R.id.menu_merge).setVisible(false);
                menu.findItem(fiek.ds.android.fieknote.R.id.menu_archive)
                        .setVisible(showArchive && !getSelectedNotes().get(0).isArchived
                                ());
                menu.findItem(fiek.ds.android.fieknote.R.id.menu_unarchive)
                        .setVisible(showUnarchive && getSelectedNotes().get(0).isArchived
                                ());
            } else {
                menu.findItem(fiek.ds.android.fieknote.R.id.menu_share).setVisible(false);
                menu.findItem(fiek.ds.android.fieknote.R.id.menu_merge).setVisible(true);
                menu.findItem(fiek.ds.android.fieknote.R.id.menu_archive).setVisible(showArchive);
                menu.findItem(fiek.ds.android.fieknote.R.id.menu_unarchive).setVisible(showUnarchive);

            }
            menu.findItem(fiek.ds.android.fieknote.R.id.menu_add_reminder).setVisible(true);
            menu.findItem(fiek.ds.android.fieknote.R.id.menu_category).setVisible(true);
            menu.findItem(fiek.ds.android.fieknote.R.id.menu_tags).setVisible(true);
            menu.findItem(fiek.ds.android.fieknote.R.id.menu_trash).setVisible(true);
        }
        menu.findItem(fiek.ds.android.fieknote.R.id.menu_select_all).setVisible(true);
    ...
    private void initSearchView(final Menu menu) {
        ...
        searchMenuItem = menu.findItem(fiek.ds.android.fieknote.R.id.menu_search);

        // Associate searchable configuration with the SearchView
        SearchManager searchManager = (SearchManager) mainActivity.getSystemService(Context.SEARCH_SERVICE);
        searchView = (SearchView) MenuItemCompat.getActionView(menu.findItem(fiek.ds.android.fieknote.R.id.menu_search));
    ...
    private void setActionItemsVisibility(Menu menu, boolean searchViewHasFocus) {
        ...
        menu.findItem(fiek.ds.android.fieknote.R.id.menu_search).setVisible(!drawerOpen);
		menu.findItem(fiek.ds.android.fieknote.R.id.menu_filter).setVisible(!drawerOpen && !filterPastReminders && navigationReminders &&
				!searchViewHasFocus);
		menu.findItem(fiek.ds.android.fieknote.R.id.menu_filter_remove).setVisible(!drawerOpen && filterPastReminders && navigationReminders
				&& !searchViewHasFocus);
		menu.findItem(fiek.ds.android.fieknote.R.id.menu_filter_category).setVisible(!drawerOpen && !filterArchivedInCategory &&
				navigationCategory && !searchViewHasFocus);
		menu.findItem(fiek.ds.android.fieknote.R.id.menu_filter_category_remove).setVisible(!drawerOpen && filterArchivedInCategory &&
				navigationCategory && !searchViewHasFocus);
		menu.findItem(fiek.ds.android.fieknote.R.id.menu_sort).setVisible(!drawerOpen && !navigationReminders && !searchViewHasFocus);
        menu.findItem(fiek.ds.android.fieknote.R.id.menu_expanded_view).setVisible(!drawerOpen && !expandedView && !searchViewHasFocus);
        menu.findItem(fiek.ds.android.fieknote.R.id.menu_contracted_view).setVisible(!drawerOpen && expandedView && !searchViewHasFocus);
        menu.findItem(fiek.ds.android.fieknote.R.id.menu_empty_trash).setVisible(!drawerOpen && navigationTrash);
        menu.findItem(fiek.ds.android.fieknote.R.id.menu_tags).setVisible(searchViewHasFocus);
    }


    @Override
    public boolean onOptionsItemSelected(final MenuItem item) {
        Integer[] protectedActions = {fiek.ds.android.fieknote.R.id.menu_empty_trash};
        if (Arrays.asList(protectedActions).contains(item.getItemId())) {
        ...
        if (actionMode == null) {
            switch (item.getItemId()) {
                case android.R.id.home:
                    ...
                    break;
				case fiek.ds.android.fieknote.R.id.menu_filter:
					filterReminders(true);
					break;
				case fiek.ds.android.fieknote.R.id.menu_filter_remove:
					filterReminders(false);
					break;
				case fiek.ds.android.fieknote.R.id.menu_filter_category:
					filterCategoryArchived(true);
					break;
				case fiek.ds.android.fieknote.R.id.menu_filter_category_remove:
					filterCategoryArchived(false);
					break;
                case fiek.ds.android.fieknote.R.id.menu_tags:
                    filterByTags();
                    break;
                case fiek.ds.android.fieknote.R.id.menu_sort:
                    initSortingSubmenu();
                    break;
                case fiek.ds.android.fieknote.R.id.menu_expanded_view:
                    switchNotesView();
                    break;
                case fiek.ds.android.fieknote.R.id.menu_contracted_view:
                    switchNotesView();
                    break;
                case fiek.ds.android.fieknote.R.id.menu_empty_trash:
                    emptyTrash();
                    break;
            }
        } else {
            switch (item.getItemId()) {
                ...
                case fiek.ds.android.fieknote.R.id.menu_share:
                    share();
                    break;
                case fiek.ds.android.fieknote.R.id.menu_merge:
                    merge();
                    break;
                ...
                case fiek.ds.android.fieknote.R.id.menu_delete:
                    deleteNotes();
                    break;
                case fiek.ds.android.fieknote.R.id.menu_select_all:
                    selectAllNotes();
                    break;
                case fiek.ds.android.fieknote.R.id.menu_add_reminder:
                    addReminders();
                    break;
...
//                    break;
    ...
    private void addReminders() {
        Intent intent = new Intent(FiekNote.getAppContext(), SnoozeActivity.class);
    ...
    private void checkSortActionPerformed(MenuItem item) {
        if (item.getGroupId() == Constants.MENU_SORT_GROUP_ID) {
			...
			((FiekNote) getActivity().getApplication()).getAnalyticsHelper().trackActionFromResourceId(getActivity(),
					item.getItemId());
		}
    }
    ...
    private void emptyTrash() {
        new MaterialDialog.Builder(mainActivity)
                .content(fiek.ds.android.fieknote.R.string.empty_trash_confirmation)
    ...
    public void toggleSearchLabel(boolean activate) {
        if (activate) {
            searchQueryView.setText(Html.fromHtml(getString(fiek.ds.android.fieknote.R.string.search) + ":<b> " + searchQuery + "</b>"));
    ...
    public void onEvent(NotesLoadedEvent notesLoadedEvent) {
        int layoutSelected = prefs.getBoolean(Constants.PREF_EXPANDED_VIEW, true) ? fiek.ds.android.fieknote.R.layout.note_layout_expanded
                : fiek.ds.android.fieknote.R.layout.note_layout;
        ...
        animate(progress_wheel).setDuration(getResources().getInteger(fiek.ds.android.fieknote.R.integer.list_view_fade_anim)).alpha(0);
        animate(list).setDuration(getResources().getInteger(fiek.ds.android.fieknote.R.integer.list_view_fade_anim)).alpha(1);
    ...
    public void trashNotes(boolean trash) {
        ...
        // Advice to user
        if (trash) {
            mainActivity.showMessage(fiek.ds.android.fieknote.R.string.note_trashed, ONStyle.WARN);
        } else {
            mainActivity.showMessage(fiek.ds.android.fieknote.R.string.note_untrashed, ONStyle.INFO);
        }

        // Creation of undo bar
        if (trash) {
            ubc.showUndoBar(false, selectedNotesSize + " " + getString(fiek.ds.android.fieknote.R.string.trashed), null);
            fab.hideFab();
            undoTrash = true;
        } else {
    ...
    private void selectAllNotes() {
        for (int i = 0; i < list.getChildCount(); i++) {
            LinearLayout v = (LinearLayout) list.getChildAt(i).findViewById(fiek.ds.android.fieknote.R.id.card_layout);
            // Checks null to avoid the footer
            if (v != null) {
                v.setBackgroundColor(getResources().getColor(fiek.ds.android.fieknote.R.color.list_bg_selected));
            }
        }
        selectedNotes.clear();
        for (int i = 0; i < listAdapter.getCount(); i++) {
    ...
    public void archiveNotes(boolean archive) {
        ...
        // Advice to user
        int msg = archive ? fiek.ds.android.fieknote.R.string.note_archived : fiek.ds.android.fieknote.R.string.note_unarchived;
        Style style = archive ? ONStyle.WARN : ONStyle.INFO;
        mainActivity.showMessage(msg, style);

        // Creation of undo bar
        if (archive) {
            ubc.showUndoBar(false, selectedNotesSize + " " + getString(fiek.ds.android.fieknote.R.string.archived), null);
            fab.hideFab();
            undoArchive = true;
        } else {
            getSelectedNotes().clear();
    ...
    private void categorizeNotesExecute(Category category) {
        ...
        String msg;
        if (category != null) {
            msg = getResources().getText(fiek.ds.android.fieknote.R.string.notes_categorized_as) + " '" + category.getName() + "'";
        } else {
            msg = getResources().getText(fiek.ds.android.fieknote.R.string.notes_category_removed).toString();
        }
        mainActivity.showMessage(msg, ONStyle.INFO);

        // Creation of undo bar
        if (category == null) {
            ubc.showUndoBar(false, getString(fiek.ds.android.fieknote.R.string.notes_category_removed), null);
            fab.hideFab();
            undoCategorize = true;
    ...
    private void tagNotesExecute(List<Tag> tags, Integer[] selectedTags, Integer[] preSelectedTags) {
        ...
        mainActivity.showMessage(fiek.ds.android.fieknote.R.string.tags_added, ONStyle.INFO);
    }
...
//	}
    ...
    public void merge() {
        new MaterialDialog.Builder(mainActivity)
                .title(fiek.ds.android.fieknote.R.string.delete_merged)
                .positiveText(fiek.ds.android.fieknote.R.string.ok)
                .negativeText(fiek.ds.android.fieknote.R.string.no)
                .callback(new MaterialDialog.ButtonCallback() {
                    @Override
                    public void onPositive(MaterialDialog dialog) {
