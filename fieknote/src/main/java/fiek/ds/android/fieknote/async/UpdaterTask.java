...
import fiek.ds.android.fieknote.utils.ConnectionManager;
...
public class UpdaterTask extends AsyncTask<String, Void, Void> {
	...
	protected void onPreExecute() {
		...
		if (FiekNote.isDebugBuild() || !ConnectionManager.internetAvailable(FiekNote.getAppContext()) || now < prefs.getLong(Constants
				...
				.callback(new MaterialDialog.ButtonCallback() {
					...
					public void onPositive(MaterialDialog materialDialog) {
						if (isGooglePlayAvailable()) {
							((FiekNote)mActivity.getApplication()).getAnalyticsHelper().trackEvent(AnalyticsHelper.CATEGORIES.UPDATE, "Play Store");
							mActivityReference.get().startActivity(new Intent(Intent.ACTION_VIEW, Uri.parse
									("market://details?id=" + mActivity.getPackageName())));
						} else {
							((FiekNote)mActivity.getApplication()).getAnalyticsHelper().trackEvent(AnalyticsHelper.CATEGORIES.UPDATE, "Drive Repository");
