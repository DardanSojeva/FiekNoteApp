...
import fiek.ds.android.fieknote.utils.GeocodeHelper;
...
public class DataBackupIntentService extends IntentService implements OnAttachingFileListener {
    ...
    protected void onHandleIntent(Intent intent) {
        ...
        mNotificationsHelper = new NotificationsHelper(this)
                .createNotification(fiek.ds.android.fieknote.R.drawable.ic_content_save_white_24dp, getString(fiek.ds.android.fieknote.R.string.working), null)
...
//		exportNotes(backupDir);
        ...
        String title = getString(fiek.ds.android.fieknote.R.string.data_export_completed);
...
//        importNotes(backupDir);
        ...
        String title = getString(fiek.ds.android.fieknote.R.string.data_import_completed);
        String text = getString(fiek.ds.android.fieknote.R.string.click_to_refresh_application);
        createNotification(intent, this, title, text, backupDir);
    }
    ...
    synchronized private void importDataFromSpringpad(Intent intent) {
        ...
        try {
            importer.setZipProgressesListener(percentage -> mNotificationsHelper.setMessage(getString(fiek.ds.android.fieknote.R.string.extracted) + " " + percentage + "%").show());
            importer.doImport(backupPath);
            // Updating notification
            updateImportNotification(importer);
        } catch (ImportException e) {
            new NotificationsHelper(this)
                    .createNotification(fiek.ds.android.fieknote.R.drawable.ic_emoticon_sad_white_24dp,
                            getString(fiek.ds.android.fieknote.R.string.import_fail) + ": " + e.getMessage(), null).setLedActive().show();
            return;
        }
        ...
        for (SpringpadElement springpadElement : importer.getNotes()) {
            ...
            if (springpadElement.getType() == null) {
                Toast.makeText(this, getString(fiek.ds.android.fieknote.R.string.error), Toast.LENGTH_SHORT).show();
                continue;
            }
    ...
    private void updateImportNotification(Importer importer) {
        mNotificationsHelper.setMessage(
                importer.getNotebooksCount() + " " + getString(fiek.ds.android.fieknote.R.string.categories) + " ("
                        + importedSpringpadNotebooks + " " + getString(fiek.ds.android.fieknote.R.string.imported) + "), "
                        + +importer.getNotesCount() + " " + getString(fiek.ds.android.fieknote.R.string.notes) + " ("
                        + importedSpringpadNotes + " " + getString(fiek.ds.android.fieknote.R.string.imported) + ")").show();
    }
    ...
    synchronized private void deleteData(Intent intent) {
        ...
        String title = getString(fiek.ds.android.fieknote.R.string.data_deletion_completed);
        String text = backupName + " " + getString(fiek.ds.android.fieknote.R.string.deleted);
        createNotification(intent, this, title, text, backupDir);
    }
    ...
    private void createNotification(Intent intent, Context mContext, String title, String message, File backupDir) {
        ...
        mNotificationsHelper.createNotification(fiek.ds.android.fieknote.R.drawable.ic_content_save_white_24dp, title, notifyIntent)
    ...
    private boolean exportAttachments(File backupDir) {
        ...
        for (Attachment attachment : list) {
            ...
            mNotificationsHelper.setMessage(TextHelper.capitalize(getString(fiek.ds.android.fieknote.R.string.attachment)) + " " + exported++ + "/" + list.size())
                    .show();
        }
        return true;
    }
    ...
    private boolean importAttachments(File backupDir) {
        ...
        while (i.hasNext()) {
            try {
                ...
                mNotificationsHelper.setMessage(TextHelper.capitalize(getString(fiek.ds.android.fieknote.R.string.attachment)) + " " + imported++ + "/" + list.size())
                        .show();
            } catch (IOException e) {
                result = false;
